...
const Header = ({ activeHeading }) => {
  ...
  return (
    <>
      <div className={`${active === true ? "shadow-sm fixed top-0 left-0 z-10" : null} transition hidden 800px:flex flex-col items-center justify-between w-full bg-[#151523] py-3`}>
        <div className={`${styles.section} w-full flex items-center justify-between`}>
          <div className="flex items-center">
            <BiMenuAltLeft size={30} className="cursor-pointer text-white mr-2" />
            <Link to="/">
              <img
          ...
          <div className="w-[50%] relative animate-border inline-block bg-white bg-gradient-to-r from-[#551c2c] via-[#b131ea] to-[#f2ad55] bg-[length:400%_400%] p-[2.5px] rounded-md">
            <input
              type="text"
          ...
          <div className="flex items-center">
            <div className="relative cursor-pointer mr-[15px]" onClick={() => setOpenWishlist(true)}>
              ...
              <span className="absolute -right-2 -top-2 rounded-full bg-[red] w-4 h-4 p-0 m-0 text-white font-mono text-[12px] leading-tight text-center">
            ...
            <Link to={isSeller ? "/dashboard" : "/shop-create"} className="ml-4">
              <button className="bg-[#4e64df] text-white px-4 py-2 rounded-full text-sm flex items-center">
      ...
      {/* mobile header */}
      <div className={`${active === true ? "shadow-sm fixed top-0 left-0 z-10" : null} w-full bg-[#121212] z-50 top-0 left-0 shadow-sm 800px:hidden`}>
        <div className="w-full flex items-center justify-between p-4">
          <div>
            <BiMenuAltLeft size={40} className="ml-1 text-[#4e64df]" onClick={() => setOpen(true)} />
          </div>
          <div>
            <Link to="/">
              <img src="https://shopo.quomodothemes.website/assets/images/logo.svg" alt="" className="w-[100px]" />
            </Link>
          </div>
          <div className="flex">
            <div className="relative cursor-pointer mr-[15px]" onClick={() => setOpenCart(true)}>
        ...
        <div className="w-full px-4 pb-4">
          <div className="w-full relative animate-border inline-block bg-white bg-gradient-to-r from-[#551c2c] via-[#b131ea] to-[#f2ad55] bg-[length:400%_400%] p-[2.5px] rounded-md">
            ...
            />
            <AiOutlineSearch size={30} className="absolute right-2 top-1.5 cursor-pointer text-[#4e64df]" />
            {searchData && searchData.length !== 0 ? (
              <div className="absolute min-h-[30vh] bg-slate-50 shadow-sm-2 z-[9] p-4">
                {searchData.map((i, index) => (
                  <Link key={index} to={`/product/${i._id}`}>
                    <div className="w-full flex items-start py-3">
                      <img src={`${i.designImage?.url}`} alt="" className="w-[40px] h-[40px] mr-[10px]" />
                      <h1>{i.DesignTitle}</h1>
                    </div>
                  </Link>
                ))}
              ...
              <div className="w-full mt-5 px-4">
                <Link to={`${isSeller ? "/dashboard" : "/shop-create"}`}>
                  <div className="w-full h-[40px] bg-[#4e64df] rounded-xl flex items-center justify-center text-white font-semibold">
                    {isSeller ? "Go Dashboard" : "Artist SignUp"} <FiArrowRight className="ml-1" />
                  </div>
                </Link>
              </div>
              <br />
