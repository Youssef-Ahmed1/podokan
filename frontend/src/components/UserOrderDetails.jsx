...
const UserOrderDetails = () => {
  ...
  const fetchOrder = useCallback(() => {
    ...
    if (!isAuthenticated || !user) {
      ...
      console.log(`Fetching user order details for ID: ${id}`);
      dispatch(getOrderDetails(id));
    } else {
      console.error("UserOrderDetails: Invalid or missing order ID.");
  ...
  useEffect(() => {
    if (currentOrder) {
      console.log("User Order Details - Rendered with Order data:", {
        orderId: currentOrder._id,
        status: currentOrder.status,
        hasUserData: !!currentOrder.user,
        isCorrectUser: currentOrder.user?._id === user?._id,
        cartItemsCount: currentOrder.cart?.length || 0,
      });
    }
  ...
  useEffect(() => {
    if (detailError) {
      console.error("User Order Details Error:", detailError);
      if (!isDetailLoading) {
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-gray-600 mt-2">
          {detailError
            ? "An error occurred while loading the order."
            : `Could not load details for the requested order. It may not exist or you may not have permission.`}
        </p>
        <Link
          to="/profile"
      ...
      <div className="max-w-6xl mx-auto">
                  ...
                  <div className="w-full sm:w-24 h-24 flex-shrink-0 bg-gray-100 rounded-md flex items-center justify-center overflow-hidden border">
                    {item.designImage?.url ? (
                      <img
                        ...
                        alt={item.DesignTitle || "Design Image"}
          ...
          <div className="bg-white rounded-lg shadow-sm p-5 border border-gray-100">
            ...
            {safeOrder.shippingAddress ? (
              <div className="text-sm text-gray-700 space-y-1">
                <p>
                  {safeOrder.shippingAddress.address1 || "(No address line 1)"}
                ...
                <p className="flex items-center pt-1 gap-1.5">
                  ...
                  {safeOrder.shippingAddress.phoneNumber || "(No phone number)"}
                </p>
              </div>
            ) : (
              <div className="text-gray-500 text-sm">
                Shipping address data is missing.
              </div>
            )}
          </div>
          <div className="bg-white rounded-lg shadow-sm p-5 border border-gray-100">
        ...
        {safeOrder.statusHistory && safeOrder.statusHistory.length > 0 && (
          <div className="mt-6 bg-white rounded-lg shadow-sm p-5 border border-gray-100">
            <h3 className="text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
              <Clock className="text-blue-600 flex-shrink-0" size={18} /> Order
              History
            </h3>
            <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {[...safeOrder.statusHistory].reverse().map((s, index) => (
                <div key={index} className="flex items-start gap-3 text-sm">
                  <div className="flex flex-col items-center mt-1">
                    ...
                    ></div>
                    {index < safeOrder.statusHistory.length - 1 && (
